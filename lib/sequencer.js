...
Sequencer.prototype.multiline = function(seq, cb) {
  ...
  ps.multiline(function(err, list, raw) {
    if(typeof cb === 'function') {
      // this is a hack as _insertString does not fire with writeChars()
      if(list.length === 1 && !list[0] && !lines.length && chars.length) {
        raw = chars.join('');
        list = [raw];
  ...
  function writeChars(chars, write) {
    chars.forEach(function(c) {
      if(write) ps.readline.write(c);
      ps.readline.input.emit('keypress', c);
    })
  }
